...
const HOME_BOTTOM_DOCK_HEIGHT_PX = 70;
const SERIES_BOTTOM_DOCK_HEIGHT_PX = 82;
const CONTROLS_ABOVE_DOCK_OFFSET_PX = -14;
const META_ABOVE_DOCK_OFFSET_PX = 20;
const ACTION_RAIL_ABOVE_DOCK_OFFSET_PX = 20;

export function getFeedVisualLayout({
  pageHeight,
  mode,
  hasSeriesEpisodeBar = false
}: {
  pageHeight: number;
  mode: "home" | "series";
  hasSeriesEpisodeBar?: boolean;
}) {
  const bottomDockHeight =
    mode === "home" ? HOME_BOTTOM_DOCK_HEIGHT_PX : hasSeriesEpisodeBar ? SERIES_BOTTOM_DOCK_HEIGHT_PX : 0;
  return {
    bottomDockHeight,
    controlsBottomOffset: Math.max(0, bottomDockHeight + CONTROLS_ABOVE_DOCK_OFFSET_PX),
    metaBottomOffset: bottomDockHeight + META_ABOVE_DOCK_OFFSET_PX,
    actionRailBottomOffset: bottomDockHeight + ACTION_RAIL_ABOVE_DOCK_OFFSET_PX,
    videoHeight: Math.max(1, pageHeight - bottomDockHeight)
  };
}
