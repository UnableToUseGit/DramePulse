...
这个交互承载的不是 `爽点`、`笑点`、`甜点`、`泪点` 这类底层情绪按钮，而是更高语义的观众表达，例如玩梗、角色魅力、CP 解读、剧情预判、站队和吐槽。
...
心里话弹幕不沿用情绪按钮分类。第一版建议使用以下 intent type，仅用于算法筛选和 debug，不要求前端展示：
...
| `cp_reading` | CP、暧昧、关系解读 | 这俩绝对有事 |
| `prediction` | 剧情预判、反转预感 | 我就知道要反转 |
| `stance` | 站队、替角色说话 | 别再误会她了 |
| `complaint` | 吐槽、反讽 | 这男的也太离谱了 |
...
第一版可以用规则词表做 intent 分类，复用现有 `EXPRESSION_PATTERNS` 的一部分，并新增更适合心里话的关键词。

建议不要只按关键词强行决定输出，而是把 intent 当作候选特征：

- 一个窗口可以有多个 intent；
- 文案选择优先看是否形成“可发送短句”；
- intent 主要用于排序、debug 和后续评估。
...
LLM 是可选步骤，不负责召回，只负责在候选窗口内选句或轻量改写。
...
```json
{
  "selected_text": "她终于怼回去了",
  "source_text": "女主终于怼回去了啊",
  "intent_type": "plot_reaction",
  "reason": "短、具体，表达观众对女主反击的共鸣。"
}
```
...
4. 不使用官方口吻；
5. 不编造真实弹幕中没有依据的含义；
6. 优先压缩、修饰真实弹幕，而不是重写观点。

如果 LLM 失败或返回不合格文案，回退到规则筛选出的最高分原始弹幕。
...
6. 在算法 review 工具中展示心里话候选和 source danmaku。
