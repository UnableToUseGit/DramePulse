...
- 只在拖拽态生效，不影响正常播放；
- 当 scrub time 接近某个非 0 秒章节起点时，预览时间吸附到该章节起点；
- 吸附阈值第一版为 `0.8s`；
- 首次进入某个边界吸附范围时触发一次 `expo-haptics` selection feedback；
- 离开吸附范围后允许再次触发，避免同一边界持续震动。

章节标题需要从单个标题升级为横向 rail：

- 当前章节标题居中、白色、高权重；
- 前一章和后一章分别显示在左右两侧，浅灰色、低权重；
- 进入下一章后，rail 的当前标题随 scrub time 更新；
- 缺少邻近章节时对应侧留空，保持当前标题居中。
...
13. scrub 接近章节切换点时吸附到边界，并触发一次 haptic selection feedback；
14. 拖动时章节标题 rail 同时显示前一章、当前章、后一章；
15. Storyboard 单帧比例与源视频画面比例一致；
16. 前端 typecheck 和相关单元测试通过。
