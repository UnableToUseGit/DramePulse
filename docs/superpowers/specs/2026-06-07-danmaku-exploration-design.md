...
窗口召回还需要满足语义信号过滤：至少包含一条 `plot_reaction`、`actor_charm`、`meme` 或 `stance` 表达。纯 `哈哈哈`、`啊啊啊`、表情占位、简单情绪宣泄堆出来的窗口，即使弹幕数量和点赞较高，也不进入心里话候选召回。

### 4.3 表达意图粗分类

第一版使用规则和可选 LLM 辅助，不要求一次性精确分类全量弹幕。
...
| `meme` | 玩梗、氛围梗、口语梗 | 这谁顶得住 |
| `emotion_burst` | 底层情绪宣泄 | 笑死我了 |
| `stance` | 观点或站队表达 | 女主别原谅他 |
| `low_signal` | 缺少可用语义 | 哈哈哈 |
| `unsafe` | 攻击、低俗或不适合前端弹出 | 需要过滤的内容 |

第一版优先推荐 `plot_reaction`、`actor_charm`、`meme`。`emotion_burst` 可以用于理解用户情绪，但默认不作为心里话弹幕主候选。
