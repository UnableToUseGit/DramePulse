...
The working directory must contain chunk metadata with `chapter_id`. DramePulse maps `/api/story-qa/ask.current_episode` to LightRAG `QueryParam.current_chapter_id`, so episode 1 can only retrieve chunks, entities, and relations sourced from `chapter_id <= 1`.
...
The LightRAG backend enforces episode-level spoiler filtering when the working directory was built with `chapter_id` metadata. Legacy working directories without `chapter_id` are expected to return sparse or empty results under the optimized LightRAG filter; DramePulse does not fall back to unfiltered LightRAG retrieval. `current_time` is not used by the LightRAG backend, so second-level spoiler control requires a future time-bucket index design.
