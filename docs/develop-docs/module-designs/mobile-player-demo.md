...
Feed 每一页仍保持整屏高度，用于维持上下滑分页、吸附和预加载窗口的稳定性；但视频画布不再铺满整屏，而是只铺满底部 dock 上方的观看区。

当前布局边界：

- Home Feed 底部 tab 区高度为 74px，视频高度为 `pageHeight - 74`；
- Series Feed 存在选集条时，底部 dock 区高度为 96px，视频高度为 `pageHeight - 96`；
- 进度条、标题信息和右侧互动栏基于 dock 高度整体上移；
- 点击暂停层和长按倍速层只覆盖视频观看区或视频上方交互区，避免底部 dock 误触发播放控制。

这样做的目标是贴近真实短剧 App：视频内容在底部操作区上方结束，底部 tab、选集条和全屏/入口按钮处于黑色操作区，而不是直接压在视频画面上。

### 4.6 广告页播放行为

Series Feed 中的角色商业化广告页复用播放器基础能力，但播放完成后的行为不同于普通短剧剧集：

- 广告页使用独立广告视频源播放；
- 广告页 active 时自动播放，用户点击视频区域可暂停或继续；
- 广告页支持拖拽进度条，拖拽后从目标时间继续播放；
- 广告页播放到结尾后停在当前页并自动暂停；
- 广告页播放完不自动切到下一页，也不展示“自动播放下一集”倒计时提示；
- 广告页标题上方展示轻量购物 CTA，点击后打开商品详情弹层，且不改变当前播放/暂停状态；
- 商品详情弹层展示商品名、价格、卖点和查看按钮，暂不接入真实下单或外部电商跳转。

该行为用于避免广告素材结束后直接跳走，给用户保留查看商品信息和手动继续滑动的空间。
