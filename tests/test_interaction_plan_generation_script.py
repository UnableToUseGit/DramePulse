...
from scripts.run_interaction_plan_generation import build_ark_client
...
class InteractionPlanGenerationScriptTest(unittest.TestCase):
    def test_main_loads_ark_config_from_env_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            source_json_path.write_text(json.dumps({"danmaku": []}, ensure_ascii=False), encoding="utf-8")
            ...
            with patch("scripts.run_interaction_plan_generation.VolcArkLlmClient", FakeClient), patch(
                "scripts.run_interaction_plan_generation.InteractionPlanGenerationPipeline",
                FakePipeline,
            ):
                result = main(
                    [
                        ...
                        str(highlight_output_dir / "highlight_recognition.json"),
                    ]
                )
