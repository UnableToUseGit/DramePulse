...
class VideoProcessor:
    ...
    def _write_frame(self, frame_path: Path, frame: np.ndarray) -> None:
        """Write a frame to disk using a Windows Unicode-path safe method."""
        success, encoded = cv2.imencode(".jpg", frame)
        if not success:
            raise IOError(f"Could not encode frame as JPEG: {frame_path}")

        encoded.tofile(str(frame_path))
        if not frame_path.exists() or frame_path.stat().st_size == 0:
            raise IOError(f"Could not write frame file: {frame_path}")
