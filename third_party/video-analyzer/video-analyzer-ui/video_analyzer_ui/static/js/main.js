...
downloadLog.addEventListener('click', downloadAnalysisLog);
...
async function handleFile(file) {
    ...
    formData.append('series_name', seriesName.value.trim());
    formData.append('episode', episodeNumber.value || '1');
    ...
    try {
        ...
        if (response.ok) {
            ...
            showConfigSection(data.display_name || file.name);
        } else {
...
function showConfigSection(fileName) {
    ...
    uploadStatus.textContent = `Ready to analyze: ${fileName}`;
...
function toggleClientSettings() {
    ...
    const ollamaInputs = ollamaSettings.querySelectorAll('input, select, textarea');
    const openaiInputs = openaiSettings.querySelectorAll('input, select, textarea');

    if (client === 'ollama') {
        ollamaSettings.style.display = 'block';
        openaiSettings.style.display = 'none';
        ollamaInputs.forEach((input) => input.disabled = false);
        openaiInputs.forEach((input) => input.disabled = true);
        if (!document.getElementById('model').value) {
            document.getElementById('model').value = 'llama3.2-vision';
        }
    } else {
        ollamaSettings.style.display = 'none';
        openaiSettings.style.display = 'block';
        ollamaInputs.forEach((input) => input.disabled = true);
        openaiInputs.forEach((input) => input.disabled = false);
        if (!document.getElementById('model').value || document.getElementById('model').value === 'llama3.2-vision') {
            document.getElementById('model').value = 'gpt-5.4';
        }
    }
    updateCommandPreview();
}
...
async function downloadAnalysisLog() {
    if (!currentSession) return;
...
function resetUI() {
    ...
    configSection.style.display = 'block';
    uploadStatus.textContent = 'Upload a video before starting analysis.';
